...
const staticNotificationList: NotificationItem[] = [
  {
    title: "Welcome Admin!",
    subTitle: "Hi again!",
  },
...
const DropdownNotification = () => {
  ...
  const fetchIrrigationEvents = async () => {
    try {
      ...
      const data: IrrigationEvent[] = await response.json();

      // Filter events for today and not done
      const today = new Date().toISOString().split('T')[0];
      const todayEvents = data.filter(event =>
          event.eventDate.startsWith(today) && !event.done
      );
      ...
      const eventNotifications = todayEvents.map(event => ({
        ...
        subTitle: `Amount: ${event.irrigationAmount}mm - ${event.eventDate.includes('T') ? event.eventDate.split('T')[1].substring(0, 5) : '00:00'}
`,
        isIrrigationEvent: true
      }));

      setNotifications(eventNotifications);
      setNotifying(eventNotifications.length > 0);
    } catch (err) {
  ...
  const markEventAsDone = async (eventId: number) => {
    try {
      ...
      // Remove the completed notification
      setNotifications(prev => prev.filter(notif => notif.id !== eventId));
      setNotifying(notifications.length > 1);
    } catch (err) {
  ...
  const handleNotificationClick = (notification: NotificationItem) => {
    if (notification.isIrrigationEvent && notification.id) {
      markEventAsDone(notification.id);
    }
    setDropdownOpen(false);
  };

  const notificationList = isAdminRoute ? staticNotificationList : notifications;
                          ...
                          notificationList.map((item, index) => (
                              <li key={index}>
                                <button
                                    ...
                                    className="flex w-full items-center gap-4 rounded-[10px] p-2.5 hover:bg-gray-2 dark:hover:bg-dark-3"
                                >
                        <span className="block">
                          ...
                          <span className="block text-body-sm font-medium text-dark-5 dark:text-dark-6">
                            {item.subTitle}
                                  ...
                                  {item.isIrrigationEvent && (
                                      <span className="ml-auto text-xs text-primary">Mark as done</span>
                                  )}
                                </button>
                              </li>
                          ))
                      ) : (
                          <li className="text-center py-4 text-dark-5 dark:text-dark-6">
                            No notifications
                          </li>
                      )}
                    </ul>
